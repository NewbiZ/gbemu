#include <gbemu/mmu.h>

#include <gbemu/cartridge.h>

#include <cstring>
#include <cassert>

namespace gbemu {
    
const uint8_t MMU::bios[] = {
    
};

MMU::MMU()
{
}

MMU::~MMU()
{
}

void MMU::powerOn(const CPU& cpu, const GPU& gpu, const Cartridge& cart)
{
    // Zero out all RAM on startup
    std::memset(memory_, 0, sizeof(memory_));
    // First 256kB of memory are initially mapped to
    // GB BIOS. Once read, BIOS is unmapped and ROM
    // takes its place.
    isBiosMapped_ = true;
}

void MMU::powerOff()
{
}

uint8_t MMU::readByte(uint16_t address) const
{
    // BIOS OR BANK 0
    if (address<0x100)
    {
        if (isBiosMapped_)
            return MMU::bios[address];
        else
        return cart_->data()[address];
    }
    // BANK 0
    else if (address<0x3FFF)
    {
        return cart_->data()[address];
    }
    // BANK 1-N
    else if (address<0x7FFF)
    {
        return cart_->data()[address];
    }
    // Graphics RAM
    else if (address<0x9FFFF)
    {
        assert(false && "error: graphics RAM not implemented");
    }
    // Cartridge RAM
    else if (address<0xBFFF)
    {
        assert(false && "error: cartridge RAM not implemented");
    }
    // Working RAM
    else if (address<0xDFFF)
    {
        assert(false && "error: working RAM not implemented");
    }
    // Working RAM shadow
    else if (address<0xFDFF)
    {
        assert(false && "error: working RAM not implemented");
    }
    // Sprite RAM shadow
    else if (address<0xFE9F)
    {
        assert(false && "error: sprite RAM not implemented");
    }
    // Memory-mapped I/O devices
    else if (address<0xFF7F)
    {
        assert(false && "error: memory mapped I/O not implemented");
    }
    // Zero-page RAM
    else if (address<0xFFFF)
    {
        assert(false && "error: zero-page RAM not implemented");
    }
}

uint16_t MMU::readWord(uint16_t address) const
{
    return 0;
}

void MMU::readByte(uint16_t address, uint8_t value)
{
}

void MMU::readWord(uint16_t address, uint16_t value)
{
}

void MMU::dump(std::ostream& stream) const
{
}

} // namespace gbemu

